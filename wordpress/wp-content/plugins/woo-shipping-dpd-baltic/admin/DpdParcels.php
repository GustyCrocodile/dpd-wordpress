...
class DpdParcels extends WC_Shipping_Method {
	...
	public function is_available( $package ) {
		...
		$countries = [ 'lt', 'lv', 'ee', 'dk', 'be', 'fi', 'fr', 'de', 'lu', 'nl', 'es', 'se', 'ch', 'gb' ];
		$countries_baltic = [ 'lt', 'lv', 'ee' ];
		$countries_pt = 'pt';
    ...
    else if (in_array( $shipping_country, $countries)) {
        $max_pudo_weight = 20 * $divider;
    }
    else if ($shipping_country === $countries_pt) {
    ...
    switch ($dpd_parcel_distribution) {
        ...
        case 2:
            ...
            if(max($total_weight_of_cart) <= $max_pudo_weight) {
                return $available;
                break;
              }
            else{
